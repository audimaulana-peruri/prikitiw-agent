...
let apiKey = process.env.INADIGITAL_API_KEY;

async function createClientAndValidate() {
  while (true) {
    if (!apiKey) {
    ...
    try {
      // Lightweight validation request. The banner is NOT shown until this succeeds.
...
const client = await createClientAndValidate();
